...
class Aggregate(nn.Module):
    def __init__(self,features,upsample='ofu',upnorm=None,upact='gelu',resnorm=None,resact='gelu',forward='refinenet',expand=False,align_corners=False,**kargs):
        super().__init__()
        ...
        if upsample == 'ofu':
            self.up = OFU(features, out_features, ofu_grid='geo', norm=upnorm, act=upact, force=False)
        elif upsample == 'inter':
...
class Fusion(nn.Module):
    def __init__(self,in_channels=[256, 512, 768, 768],out_channels=256,upsample='ofu',expand=False,align_corners=False):
        super().__init__()
        if expand:
...
class Head(nn.Module):
    def __init__(self,channels=256, non_negative=True, start=0.0, end=10.0, **kargs):
        super().__init__()
