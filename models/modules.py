...
from .geometry.geometry import xy_grid,uv_grid,xyz_grid
...
def spherical_upsample(x,scale):
    ...
    if d > 0: x = x.view([b,c,d,h*scale,w*scale]).contiguous()
    return x

def spherical_interpolate(x,up,down):
    if up > down:
        s = up // down
        x = _make_pad(1)(x)
