...
LUT = np.uint8(np.array(turbo_colormap_data,dtype=np.float32)*255)

def turbo_colormap(img,depth_max=10.0,th=np.Inf,keep_zero=True):
    assert len(img.shape) == 2
    invalid = np.logical_or(img>th,img<=0) if keep_zero else img>th
    img = np.int32(np.clip(img,0,depth_max)*255/depth_max)
    img = LUT[img]
    img[invalid] = 0
    return img
    
def tensor2img(image_tensor, imtype='turbo', cv2order=True, scale=-1,mask=None):
    if 'max' in _args['data']:
        th = _args['data']['max']
    else:
        th = 10.0
    ...
    if imtype == np.uint8:
        ...
        image_numpy = np.int32(np.clip(image_numpy/th*255,0,255))
        image_numpy = LUT[image_numpy]
        image_numpy[invalid] = 0
    # elif imtype == 'label':
    #     assert len(image_numpy.shape) == 2
    #     image_numpy = np.int32(image_numpy)
    #     invalid = image_numpy >= 13
    #     image_numpy[invalid] = 13
    #     image_numpy = colors_13[image_numpy]
    elif imtype == 'bool':
        assert len(image_numpy.shape) == 2
        image_numpy = np.uint8(image_numpy.astype(bool))*255
    # elif imtype == 'raw':
    #     image_numpy = np.uint16(np.clip(image_numpy*1000, 0, 65535))
...
def theta(h,w,device=None,repeat=None):
    ...
    if device is None:
        ...
        u = (x.to(device)-(w-1)/2)/w*(2*np.pi)
        v = -(y.to(device)-(h-1)/2)/h*np.pi
    return u,v

def depth2xyz(depth):
    assert len(depth.shape) == 2
    h,w = depth.shape
    u,v = theta(h,w,depth.device)
    x = torch.cos(v)*torch.sin(u)
    y = torch.cos(v)*torch.cos(u)
    z = torch.sin(v)
    xyz = torch.stack([x,y,z])
    XYZ = xyz*depth.unsqueeze(0)
    return XYZ

def depth2ply(depth,rgb,mask=None,text=False):
    # depth [H,W]
    # rgb [3,H,W]
    # mask None or [H,W]
    XYZ = depth2xyz(depth)
