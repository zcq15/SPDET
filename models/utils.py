...
def interpolate_r(x):
    if x < 0.0: return 0.0
    elif x > 1.0: return turbo_colormap_data[-1][0]
    else:
        a = int(x*255.0)
        b = min(255, a + 1)
        f = x*255.0 - a
...
def interpolate_g(x):
    if x < 0.0: return 0.0
    elif x > 1.0: return turbo_colormap_data[-1][1]
    else:
        a = int(x*255.0)
        b = min(255, a + 1)
        f = x*255.0 - a
        return turbo_colormap_data[a][1] + (turbo_colormap_data[b][1] - turbo_colormap_data[a][1]) * f

@np.vectorize
def interpolate_b(x):
    if x < 0.0: return 0.0
    elif x > 1.0: return turbo_colormap_data[-1][2]
    else:
        a = int(x*255.0)
        b = min(255, a + 1)
        f = x*255.0 - a
        return turbo_colormap_data[a][2] + (turbo_colormap_data[b][2] - turbo_colormap_data[a][2]) * f

def turbo_colormap(x,dtype=np.uint16):
    assert len(x.shape) == 2
    assert dtype in [np.uint8, np.uint16]
    turbo_r = interpolate_r(x)
    turbo_g = interpolate_g(x)
    turbo_b = interpolate_b(x)
    turbo_rgb = np.stack([turbo_r,turbo_g,turbo_b],axis=2)
    if dtype == np.uint8:
        return np.uint8(turbo_rgb*255)
    else:
        return np.uint16(turbo_rgb*65535)
    
def tensor2img(image_tensor, imtype='turbo', cv2order=True, scale=-1, mask=None, th=None):
    if th is None: th = 10.0

    assert imtype in [np.uint8,np.uint16,'turbo','bool']
    ...
    if scale>0: image_numpy = image_numpy*scale

    if imtype == np.uint8:
        image_numpy = np.uint8(np.clip(image_numpy, 0, 255))

    elif imtype == np.uint16:
        image_numpy = np.uint16(np.clip(image_numpy, 0, 65535))

    elif imtype == 'turbo':
        assert len(image_numpy.shape) == 2
        invalid = image_numpy <= 0
        image_numpy = image_numpy/th
        image_numpy = turbo_colormap(image_numpy,np.uint16)
        image_numpy[invalid] = 0

    elif imtype == 'bool':
        assert len(image_numpy.shape) == 2
        image_numpy = np.uint8(image_numpy.astype(bool))*255

    if len(image_numpy.shape) == 3 and cv2order:
        image_numpy = cv2.cvtColor(image_numpy,cv2.COLOR_RGB2BGR)

    if not mask is None:
        image_numpy[np.logical_not(mask.cpu().numpy())] = 0
